import HexGrid from '@/components/common/Hex/HexGrid';
...
export default class Hexagon {
	...
	drawSelectHex(ctx, color = '#88c8b4') {
		ctx.save();
		ctx.beginPath();
	...
	drawHex(ctx, color = '#ff9f48') {
		...
		// ctx.save();
		// ctx.beginPath();
		// ctx.arc(this.TopLeftPoint.X, this.TopLeftPoint.Y, 1, 0, Math.PI * 2, false);
		// ctx.fillStyle = '#04fa24';
		// ctx.fill();
		// ctx.closePath();
		// ctx.beginPath();
		// ctx.arc(this.BottomRightPoint.X, this.BottomRightPoint.Y, 1, 0, Math.PI * 2, false);
		// ctx.fillStyle = '#a70186';
		// ctx.fill();
		// ctx.closePath();
		// ctx.restore();
	}
	...
	getCorner(corner) {
		...
		const offset = HexGrid.expression === HexGrid.Orientation.Normal ? 90 : 90;
		...
		// 화면 밖으로~ 5는 값 조절 때문에 쓰임 변동 가능
		if (
			hY <= this.rectPos.y ||
			hX <= this.rectPos.x ||
			hX >= this.rectBox.w + this.rectPos.x + this.correction ||
			hY >= this.rectBox.h + this.rectPos.y + this.correction
		) {
			this.isOverHex = true;
		}
	...
	getCenter() {
		this.TopLeftPoint.X = this.center.X + this.correction - this.HexSide;
		this.TopLeftPoint.Y = this.center.Y + this.correction - this.HexSide;
		this.BottomRightPoint.X = this.center.X - this.correction + this.HexSide;
		this.BottomRightPoint.Y = this.center.Y - this.correction + this.HexSide;
		// 	= new HexPointDrawPos(
