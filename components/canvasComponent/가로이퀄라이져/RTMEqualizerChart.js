var RTMEqualizerChart = (function () {
            ...
            RTMEqualizerChart.prototype.initResizeProperty = function () {
                ...
                // this.ratioW = this.targetW - (this.targetW - this.beforeW)
                this.beforeW = this.targetW;
            ...
            RTMEqualizerChart.prototype.createCanvas = function () {
                ...
                this.ctxAni = this.canvasAni.getContext("2d");
                this.ctxEvent = this.canvasEvent.getContext('2d');

                this.canvasList = [this.canvasBG, this.canvasAni, this.canvasEvent];
                this.canvasCls = ["canvasBG", "canvasAni", "canvasEvent"];
                this.ctxList = [this.ctxBG, this.ctxAni, this.ctxEvent];
            };
            // 캔버스 초기화
            ...
            RTMEqualizerChart.prototype.initComponent = function (args) {
                ...
                this.eqMoveRect = {
                    ...
                    draw: function () {
                        ...
                        // me.ctxBG.globalAlpha = 0.5;
                        me.ctxAni.beginPath();
                        me.ctxAni.fillStyle = this.color;
                        me.ctxAni.shadowColor = '#1d0b15';
                        me.ctxAni.shadowBlur = 5;
                        me.ctxAni.fillRect(this.x, this.y, this.w, this.h);
                        me.ctxAni.fill();
                        me.ctxAni.restore();
                    }
                ...
                this.eqName = {
                    ...
                    otherDraw: function () {
                        me.ctxAni.save();
                        me.ctxAni.font = this.fontSize + this.font;
                        me.ctxAni.fillStyle = this.fontColor;
                        me.ctxAni.fillText(this.text, this.x, this.y);
                        me.ctxAni.restore();
                    }
                }
            };
            RTMEqualizerChart.prototype.MouseEventCall = function () {
                ...
                this.canvasEvent.addEventListener('mousemove', function (evt) {
                    ...
                    if (entity) {
                        ...
                        self.el.dom.style.cursor = 'pointer';
                        // 툴팁 이벤트 건다.
                        // alarmLevel !== 0 일때만 실행(normal 이 아닐때만)
                        if (entity.bizData.ALARM_LEVEL > 0 && entity.bizData.ALARM_LEVEL < 3) {
                            // debugger;
                            // serverId = entity.serverId;
                            bizName = entity.bizName;

                            // if (self.data.alarmList[serverId]) {
                            //     self.showAlertDetail(entity.sx, entity.sy, self.data.alarmList[serverId], entity.serverName);
                            // }
                            self.showAlertDetail(entity.sx, entity.sy, bizName, offset, entity.canvas);
                        } else {
                            // normal
                            // if (self.isToolTip) {
                            //     var contents = entity.serverName;
                            //     var wn = $(".rtmKTFCTableChart");
                        ...
                        self.el.dom.style.cursor = 'default';
            ...
            RTMEqualizerChart.prototype.taskNameLen = function (area) {


                var isOk = true;
                var txt, txtByte, txtWidth, txtLength, textSpace, ix, expre, isReduce;
                ix = 0;
                expre = '..';
                isReduce = false;
                // 무한루프 조심
                while (isOk) {
                    ...
                    txtWidth = this.eqName.textW().width;
                    ...
                    if (textSpace < txtWidth) {
                        this.eqName.text = txt.substr(0, (txtLength - (ix)));
                        isReduce = true;
                    } else {
                        if(isReduce){
                            return txt + expre;
            ...
            RTMEqualizerChart.prototype.taskName = function (bizName, txtspace, alarmLevel) {
                ...
                var textX = (this.sx) + this.logo.radius + 5;

                this.eqName.text = bizName;
                var textW = this.eqName.textW().width;


                // var checkTextSize = textX + (txtspace) - 20;
                // if (textW <= checkTextSize) {
                //     this.eqName.text = bizName;
                // } else {
                var textSpace = txtspace * 3.5;
                this.eqName.text = this.taskNameLen(textSpace);
                // }
                //(this.eqName.x + txtspace * 3.5)

                this.eqName.x = textX;
                this.eqName.y = (this.sy + this.textY) + (this.alarmBox.height / 2);
                // this.ctxBG.rect(textX, (this.sy - this.textY - 6) + (this.alarmBox.height / 2), textSpace, 25);
                // this.ctxBG.stroke();
            ...
            RTMEqualizerChart.prototype.activeBackground = function (txtspace, alarmLevel) {
                // 액티브트랜잭션 배경
                this.activeX = (this.eqName.x + txtspace * 3.5); //+ this.activeSumTextWidth);
                this.activeY = (this.eqName.y - this.textY);
                this.activeEndX = this.alarmBox.lineLength - 20; // 숫자 4자리 넓이가 20px임
                ...
                this.ctxBG.lineTo(this.activeEndX, (this.activeY));
                // this.ctxBG.stroke();
                this.ctxBG.restore();
            ...
            RTMEqualizerChart.prototype.ratioCalu = function (data, bizName) {
                ...
                this.rectMaxCount = (this.activeEndX - this.activeX) / 4;
            ...
            RTMEqualizerChart.prototype.activeRect = function (data) {
                ...
                this.activeRectLastX = posX;

            };
            RTMEqualizerChart.prototype.moveRect = function (args, valX) {
                switch (args.alarmLevel) {
                    case 0:
                        this.eqMoveRect.color = this.colors.N;
                        break;
                    case 1:
                        this.eqMoveRect.color = this.colors.W;
                        break;
                    case 2:
                        this.eqMoveRect.color = this.colors.C;
                        break;
                }

                this.eqMoveRect.x = args.activeRectLastX + valX;
                this.eqMoveRect.y = args.activeY - (args.eqPos);
                this.eqMoveRect.draw();
            };
            RTMEqualizerChart.prototype.activeTotalValue = function (args, valX) {
                if (args.alarmLevel === 3) {
                    this.eqName.fontColor = this.serverDown.SERVER_DOWN_LIGHT
                } else {
                    this.eqName.fontColor = this.colors.fontColor;
                }

                this.eqName.x = args.activeRectLastX + this.eqMoveRect.w + valX + 2 // 2.4 간격 동일하게 맞추기 위해
                // console.log((this.sx), this.logo.radius, txtspace, this.eqName.textW().width);
                this.eqName.y = (args.sy + args.textY) + (this.alarmBox.height / 2);
                this.eqName.fontColor = '#DBDBDB';
                this.eqName.text = this.getComma(args.activeCnt);
                this.eqName.otherDraw();

            };
            RTMEqualizerChart.prototype.groupTitle = function (groupName) {
                this.eqName.text = groupName;
            ...
            RTMEqualizerChart.prototype.initMoveBox = function () {
                this.moveBox = (function () {

                    var moveBox = function (args) {
                        if (!(this instanceof moveBox)) {
                            return new moveBox(args);
                        }
            ...
            RTMEqualizerChart.prototype.canvasItemDraw = function () {
                ...
                for (var ix = 0; ix < bizData.length; ix++) {
                    ...
                    // 그룹 타이틀
                    this.groupTitle(bizData[ix].group_name);
                    ...
                    for (var iy = 0; iy < bizkey.length; iy++) {
                        ...
                        // 전체에 대한 비율 값 정보 추출
                        if (alarmLevel !== 3) {
                            this.activeRect(this.ratioCalu(bizData, bizName));
                        }

                        // 애니메이션 좌우
                        // if(iy ===0) this.basicAniMoveRect(bizData.ACTIVE_TXN_COUNT, txtspace, alarmLevel);
                        // const start = this.activeRectLastX;
                        // const end = this.activeRectLastX + (this.eqRect.w * 4);
                        this.aniInfo = new this.moveBox({
                            activeRectLastX: this.activeRectLastX,
                            activeY: this.activeY,
                            eqPos: this.eqPos,
                            activeCnt: bizData.ACTIVE_TXN_COUNT,
                            alarmLevel: alarmLevel,
                            sy: this.sy,
                            sx: this.sx,
                            textY: this.textY,
                        });
                        this.moveRectEntities.push(this.aniInfo);

                        // this.moveRect(start);
                        // this.activeTotalValue(bizData.ACTIVE_TXN_COUNT, start, alarmLevel);
                ...
                // 좌우 애니메이션 rect
                this.basicAniMoveRect();



            }
            RTMEqualizerChart.prototype.RectEntitieDraw = function (valX) {

                this.clear(this.ctxAni);
                for (var ix = 0, ixlen = this.bizEntities.length; ix < ixlen; ix++) {
                    var entitieInfo = this.moveRectEntities[ix];

                    if (entitieInfo.alarmLevel !== 3) {
                        this.moveRect(entitieInfo, valX);
                        this.activeTotalValue(entitieInfo, valX);
                    }

                }
            }
            RTMEqualizerChart.prototype.basicAniMoveRect = function (activeCNt, txtspace, alarmLevel) {
                var me = this;
                const start = 0;
                const end = 5;
                var startX = start;
                ...
                if (this.moveRectEntities.length === 0) return;
